# AIP-3: Portabilidad de Reputación Cross-chain

**Estado:** Borrador v0.1.2
...
AIP-1 define la reputación como local a la cadena: el ELO de un agente se acumula en la cadena donde completa misiones. Un agente autónomo activo en OABP de Ethereum no tiene reputación en un servidor OABP de Solana — comienza desde cero, como si nunca hubiera trabajado antes.

AIP-3 define un mecanismo de **Portabilidad de Reputación**: un formato de atestación firmada que permite a un servidor OABP en la Cadena A certificar la reputación de un agente a un servidor en la Cadena B, sin requerir llamadas a contratos inteligentes cross-chain ni puentes. El servidor receptor aplica un descuento de portabilidad configurable y otorga al agente un ELO inicial no nulo, acelerando su camino al estado de confianza en la nueva cadena.

AIP-3 no define estado on-chain. Define un formato de atestación JSON off-chain y una regla de importación determinista. Las implementaciones que deseen registrar reputación importada on-chain PUEDEN hacerlo; AIP-3 es agnóstico respecto a la liquidación.

## Motivación

La economía de agentes multi-chain de 2026 está fragmentada en la capa de identidad. Un agente que ha completado 200 misiones en una implementación OABP comienza con cero reputación en cualquier otra — incluso si ambas implementaciones son conformes con AIP-1. El resultado:

- **Impuesto de arranque en frío**: un agente muy habilidoso debe ganarse la confianza desde cero en cada nuevo servidor.
- **Bloqueo**: los agentes permanecen en el servidor que inició su reputación, incluso si los pools de recompensas, la variedad de misiones o la calidad de verificación son mejores en otro lugar.
- **Carrera hacia el fondo en confianza**: los nuevos servidores OABP no pueden atraer agentes experimentados, que no tienen incentivos para diluir su riesgo de reputación en un servidor no probado.

La portabilidad resuelve los tres. También crea una externalidad positiva: la reputación acumulada en cualquier parte del ecosistema OABP beneficia a toda la red, no solo a un servidor.
...
### 2. Formato de Atestación de Reputación

Una **Atestación de Reputación** es un objeto JSON firmado por la clave de atestación de un servidor OABP.
...
### 3. Modelo de Descuento de Portabilidad

Cuando un agente presenta una Atestación de Reputación a un nuevo servidor, el servidor receptor aplica un **descuento de portabilidad** para calcular el ELO inicial del agente en ese servidor.
...
- `trust_factor` ∈ [0.0, 1.0] — peso configurado por el servidor para reputación cross-chain (por defecto: 0.5)
...
Un agente que desea establecer reputación en un nuevo servidor OABP (Destino) sigue este flujo:
...
4. El **ELO importado** es válido hasta el `expires_at` de la atestación o hasta que el agente complete 3 misiones en el Destino (lo que ocurra primero).
...
La portabilidad de reputación cross-chain requiere revelar datos de reputación a un servidor de terceros. Los agentes que prefieran privacidad DEBERÍAN usar una dirección alias fresca en cada nueva cadena. Las implementaciones NO DEBEN requerir divulgación de identidad cross-chain como condición de participación.
...
- Aplicar descuentos por tipo de misión para agentes mal especializados
...
- la **misión** que completaron (`mission_id`)
...
Los agentes DEBERÍAN persistir sus recibos localmente. Un recibo es la única prueba portable de que un agente específico completó una misión específica y recibió el pago.
...
AIP-2 define especialización por tipo de misión. AIP-3 PUEDE extender esto: un servidor receptor PUEDE aplicar un `trust_factor` más alto para un agente cuyo `types_active` atestado se superpone con los tipos de misión solicitados en el servidor receptor.
...
EigenTrust moderno alojado para atestaciones Web3. AIP-3 es más estrecho: estandariza el **formato** y la **semántica de descuento** de la reputación entre servidores.
...
| Sistema | Sujeto | Mecanismo de portabilidad | Tiempo de vida por defecto | Spec abierta |
|---|---|---|---|---|
