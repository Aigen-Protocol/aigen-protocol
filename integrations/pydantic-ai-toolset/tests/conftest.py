"""Pytest bootstrap for the pydantic_ai_oabp test-suite.

Adds the project root to ``sys.path`` so tests import the local
``pydantic_ai_oabp`` package (which in turn falls back to its vendored ``oabp``
...
``pydantic-ai`` package (the package must work without it).
"""
...
@pytest.fixture
def make_deps():
    """Factory: build an OabpDeps wired to a RoutingFakeSession.

    Returns ``(deps, session)``. Mirrors how pydantic-ai would supply deps to a
    run, but offline — the tests then wrap ``deps`` in a ``RunContext`` and call
    the tool functions directly.
    """
    from pydantic_ai_oabp import OabpClient, OabpDeps

    def _make(routes: Dict[tuple, Any], *, agent_id: Optional[str] = None, **kw: Any):
        session = RoutingFakeSession(routes)
        client = OabpClient(session=session, agent_id=agent_id, **kw)
        deps = OabpDeps(client=client, agent_id=agent_id)
        return deps, session
...
@pytest.fixture
def make_ctx(make_deps):
    """Factory: build a (RunContext, session) over a RoutingFakeSession."""
    from pydantic_ai_oabp import RunContext

    def _make(routes: Dict[tuple, Any], *, agent_id: Optional[str] = None, **kw: Any):
        deps, session = make_deps(routes, agent_id=agent_id, **kw)
        return RunContext(deps=deps), session
