"""Resolve the OABP Python SDK that backs the Pydantic-AI integration.

The ``pydantic_ai_oabp`` tools are a thin, idiomatic wrapper over the
...
* Otherwise we transparently fall back to the copy vendored under
  :mod:`pydantic_ai_oabp._vendor.oabp`, so the integration works out-of-the-box
  with no extra install step.

Every other module in :mod:`pydantic_ai_oabp` imports the SDK symbols *from
here* rather than reaching for ``oabp`` directly, which keeps the fallback
behaviour in exactly one place. Crucially, this module imports cleanly whether
or not ``pydantic-ai`` is installed — only :mod:`pydantic_ai_oabp.toolset` and
:mod:`pydantic_ai_oabp.agent` touch ``pydantic_ai``, and they do so behind a
lazy import.
"""
...
def _load_oabp() -> Tuple[ModuleType, bool]:
    ...
    vendored = importlib.import_module("pydantic_ai_oabp._vendor.oabp")
