"""Resolve the OABP Python SDK that backs the smol-agents integration.

The smol-agents toolset is a thin, idiomatic wrapper over the *synchronous* OABP
Python SDK (the ``oabp`` package). This module is the single import seam between
the two:
...
* Otherwise we transparently fall back to the copy vendored under
  :mod:`smolagents_oabp._vendor.oabp`, so the integration works out-of-the-box
  with no extra install step.

Every other module in :mod:`smolagents_oabp` imports the SDK symbols *from here*
rather than reaching for ``oabp`` directly, which keeps the fallback behaviour
...
def _load_oabp() -> Tuple[ModuleType, bool]:
    ...
    vendored = importlib.import_module("smolagents_oabp._vendor.oabp")
