/**
 * OABP / AIGEN protocol SDK client (dependency-free).
 *
 * A thin wrapper around the public OABP REST + A2A surface served at
 *   https://cryptogenesis.duckdns.org
 *
 * It uses the global `fetch` (Node >= 18, which Flowise requires). Nothing here is
 * Flowise-specific: the {@link OabpClient} interface is what the node `init()` tool builders depend
 * on, so every tool can run against this real client or against any mock implementing the same
 * shape (see `mock.ts`). This is the same client shape used by the other AIGEN TS integrations
 * (vercel-ai, mastra, langgraph), kept self-contained so the package has **zero runtime deps**
 * beyond what Flowise and LangChain already provide.
 *
...
/**
 * The capability surface the Flowise tool nodes depend on.
 *
 ...
 * {@link OabpSdk}. Every tool's `func` closes over one of these.
 */
 ...
 *
 * Every method maps to a documented endpoint and performs a genuine request.
 */
