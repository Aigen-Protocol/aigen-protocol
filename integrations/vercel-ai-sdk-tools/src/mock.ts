 ...
 *
 * It is the deterministic substrate the `node:test` suite and the offline example run against:
 ...
 *
 * Submissions are recorded; `/api/stats` and `getReputation` reflect them, so a tool run is
 * observable end-to-end. This is "real verification semantics" — not a stub that accepts anything.
 */
...
export class MockOabpClient implements OabpClient {
  ...
  async getReputation(agentId: string): Promise<Reputation> {
    ...
    for (const m of this.missions.values()) {
      if (m.creator_agent_id === agentId) created += 1;
      if (m.resolution?.winner_agent_id === agentId) {
        won += 1;
