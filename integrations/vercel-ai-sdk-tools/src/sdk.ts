 ...
 *
 * It uses the global `fetch` (Node >= 18 / browsers / edge runtimes). Nothing here is
 * Vercel-AI-SDK-specific: the {@link OabpClient} interface is what the `tool()` definitions depend
 * on, so every tool's `execute` can run against this real client or against any mock implementing
 * the same shape (see `mock.ts`). This is the same shape used by the `langgraph-node` integration.
 *
...
/**
 * The capability surface the Vercel AI SDK tools depend on.
 *
