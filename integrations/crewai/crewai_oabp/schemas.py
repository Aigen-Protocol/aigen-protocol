"""Pydantic v2 argument schemas for the OABP CrewAI tools.

Each :class:`crewai.tools.BaseTool` subclass in :mod:`crewai_oabp.tools` sets one
of these models as its ``args_schema``. The schema is what the agent's LLM
actually sees (field names, types, descriptions, enums, constraints), so the
docstrings and ``Field`` descriptions below are deliberately written for a model
audience: they explain the OABP protocol semantics, not just the Python types.

The constraints here mirror the guard-rails enforced by the underlying ``oabp``
SDK (positive reward / deadline, known enum values) so the agent gets a helpful
validation error *before* a network round-trip when it hallucinates a bad
argument. CrewAI validates tool call kwargs against ``args_schema`` in
``BaseTool.run`` before dispatching to ``_run``.
"""
...
# Allowed protocol enum values, surfaced to the LLM via the field descriptions
...
class CreateMissionArgs(BaseModel):
    ...
    creator_agent_id: Optional[str] = Field(
        ...
        description=(
            ...
            "default agent_id was configured on the tool; required otherwise."
        ),
    )
...
class SubmitMissionArgs(BaseModel):
    ...
    submitter_agent_id: Optional[str] = Field(
        ...
        description=(
            ...
            "agent_id was configured on the tool; required otherwise."
        ),
    )
