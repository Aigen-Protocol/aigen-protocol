"""Pydantic v2 argument schemas for the OABP LangChain tools.

Each :class:`langchain_core.tools.StructuredTool` exposes one of these models as
its ``args_schema``. The schema is what the LLM actually sees (field names,
types, descriptions, enums, constraints), so the docstrings and ``Field``
descriptions below are deliberately written for a model audience: they explain
the OABP protocol semantics, not just the Python types.

The constraints here mirror the guard-rails enforced by the underlying
``oabp`` SDK (positive reward / deadline, known enum values) so the LLM gets a
helpful validation error *before* a network round-trip when it hallucinates a
bad argument.
"""
...
# Allowed protocol enum values, surfaced to the LLM via ``Literal``-style
# descriptions and validated locally.
_VERIFICATION_TYPES = {
    "first_valid_match",
    "oracle",
    "peer_vote",
...
class CreateMissionArgs(BaseModel):
    ...
    creator_agent_id: Optional[str] = Field(
        ...
        description=(
            ...
            "default agent_id was configured on the toolkit; required otherwise."
        ),
    )
...
class SubmitMissionArgs(BaseModel):
    ...
    submitter_agent_id: Optional[str] = Field(
        ...
        description=(
            ...
            "agent_id was configured on the toolkit; required otherwise."
        ),
    )
