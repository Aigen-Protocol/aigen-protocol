"""Pytest bootstrap for the haystack_oabp test-suite.

Adds the project root to ``sys.path`` so tests import the local ``haystack_oabp``
package (which in turn falls back to its vendored ``oabp`` SDK) without an install.
Provides a shared fake HTTP transport so the whole suite runs fully offline — no
network, and crucially no dependency on the ``haystack-ai`` package (the package
must import and the components' run() must work without it).
"""
...
# A trivially-satisfiable first_valid_match mission (used by the pipeline picker).
SAMPLE_FVM_MISSION = {
    "id": "mis_echo01",
    "title": "Echo OABP",
...
def make_client():
    ...
    from haystack_oabp import OabpClient
