"""Resolve the OABP Python SDK that backs the Haystack integration.

The ``haystack_oabp`` components are a thin, idiomatic wrapper over the
...
* Otherwise we transparently fall back to the copy vendored under
  :mod:`haystack_oabp._vendor.oabp`, so the integration works out-of-the-box with
  no extra install step.

Every other module in :mod:`haystack_oabp` imports the SDK symbols *from here*
...
def _load_oabp() -> Tuple[ModuleType, bool]:
    ...
    vendored = importlib.import_module("haystack_oabp._vendor.oabp")
