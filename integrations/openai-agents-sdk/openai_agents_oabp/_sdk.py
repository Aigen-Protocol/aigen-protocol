"""Resolve the OABP Python SDK that backs the OpenAI Agents SDK integration.

The ``openai_agents_oabp`` tools are a thin, idiomatic wrapper over the
...
* Otherwise we transparently fall back to the copy vendored under
  :mod:`openai_agents_oabp._vendor.oabp`, so the integration works out-of-the-box
  with no extra install step.

Every other module in :mod:`openai_agents_oabp` imports the SDK symbols *from
...
def _load_oabp() -> Tuple[ModuleType, bool]:
    ...
    vendored = importlib.import_module("openai_agents_oabp._vendor.oabp")
