...
OpenAI Agents SDK tools return their result to the model as text/JSON, so every
tool here yields a **plain, JSON-serialisable dict** trimmed to the fields a model
actually needs — never a dataclass, never an Enum. These helpers do that mapping
and are shared by all the tool closures in :mod:`openai_agents_oabp.tools`.
"""
...
def error_to_string(exc: OabpError) -> str:
    ...
    The OpenAI Agents SDK surfaces a tool's return value to the model as text, so
    instead of raising (which aborts the tool call) we return a short, structured
    one-line ``ERROR ...`` string the model can read and react to.
    """
