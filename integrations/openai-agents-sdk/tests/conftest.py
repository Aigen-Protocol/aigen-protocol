"""Pytest bootstrap for the openai_agents_oabp test-suite.

Adds the project root to ``sys.path`` so tests import the local
``openai_agents_oabp`` package (which in turn falls back to its vendored ``oabp``
...
``openai-agents`` package (the package must work without it).
"""
...
def make_client():
    ...
    from openai_agents_oabp import OabpClient
