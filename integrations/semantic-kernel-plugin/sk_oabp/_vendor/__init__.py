"""Vendored third-party packages bundled with ``sk_oabp``.

This sub-package ships a pinned copy of the **OABP Python SDK** (the ``oabp``
package) so that the Semantic Kernel plugin is self-contained and importable
...
import path; see :mod:`sk_oabp._sdk` for the resolution logic.
"""
