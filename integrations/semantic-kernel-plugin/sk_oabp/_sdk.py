"""Resolve the OABP Python SDK that backs the Semantic Kernel plugin.

The :class:`sk_oabp.OabpPlugin` is a thin, idiomatic wrapper over the
...
* If a standalone ``oabp`` distribution is installed (``pip install oabp``), it
  is used directly — the plugin tracks the user's pinned SDK version.
* Otherwise we transparently fall back to the copy vendored under
  :mod:`sk_oabp._vendor.oabp`, so the plugin works out-of-the-box with no extra
  install step.

Every other module in :mod:`sk_oabp` imports the SDK symbols *from here* rather
than reaching for ``oabp`` directly, which keeps the fallback behaviour in
exactly one place.
"""
...
def _load_oabp() -> Tuple[ModuleType, bool]:
    ...
    vendored = importlib.import_module("sk_oabp._vendor.oabp")
...
# Re-export the public SDK surface the plugin relies on. Importing these names
# here (rather than `from oabp import ...` scattered across modules) means the
# vendored-vs-installed decision is made exactly once, on first import.
