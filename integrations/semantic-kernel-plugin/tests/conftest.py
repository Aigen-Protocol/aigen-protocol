"""Pytest bootstrap for the sk_oabp test-suite.

Adds the project root to ``sys.path`` so tests import the local ``sk_oabp``
package (which in turn falls back to its vendored ``oabp`` SDK) without an
install. Provides a shared fake HTTP transport so the whole suite runs fully
offline — no network, and crucially no dependency on the ``semantic-kernel``
package (the package must work without it).
"""
...
SAMPLE_MISSION = {
    ...
    # A mission that gates submitters by reputation — the plugin must surface
...
def make_client():
    ...
    from sk_oabp import OabpClient
...
@pytest.fixture
def make_plugin(make_client):
    """Factory: build an OabpPlugin over a RoutingFakeSession-backed client."""
    from sk_oabp import OabpPlugin

    def _make(routes: Dict[tuple, Any], *, agent_id=None, **client_kwargs: Any):
        client, session = make_client(routes, agent_id=agent_id, **client_kwargs)
        plugin = OabpPlugin(client=client, agent_id=agent_id)
