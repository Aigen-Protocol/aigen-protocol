"""Pydantic v2 argument schemas for the OABP AutoGen / AG2 tool functions.

Each OABP tool callable in :mod:`autogen_oabp.tools` validates its keyword
arguments against one of these models *before* a network round-trip. AutoGen /
AG2 derives the LLM-facing JSON schema from the callable's ``Annotated`` type
hints (via ``register_function`` / ``register_for_llm``); these Pydantic models
mirror those hints and add the protocol guard-rails (positive reward / deadline,
known enum values) so a hallucinated argument fails fast with a precise message
rather than at the server.

The field descriptions are written for a model audience: they explain the OABP
protocol semantics, not just the Python types. They deliberately mirror the
schema/docstring style of ``langchain_oabp.schemas`` so the two integrations
present an identical contract to an LLM.
"""
...
# Allowed protocol enum values, surfaced to the LLM via the callable docstrings
