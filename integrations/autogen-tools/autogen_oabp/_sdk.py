"""Resolve the OABP Python SDK that backs the AutoGen / AG2 integration.

The AutoGen toolset is a thin, idiomatic wrapper over the *synchronous* OABP
...
* Otherwise we transparently fall back to the copy vendored under
  :mod:`autogen_oabp._vendor.oabp`, so the integration works out-of-the-box
  with no extra install step.

Every other module in :mod:`autogen_oabp` imports the SDK symbols *from here*
...
def _load_oabp() -> Tuple[ModuleType, bool]:
    ...
    vendored = importlib.import_module("autogen_oabp._vendor.oabp")
