 ...
 *
 * It uses the global `fetch` (Node >= 18 / browsers). Nothing here is LangGraph-specific:
 * the {@link OabpClient} interface is what the graph nodes depend on, so they can run
 * against this real client or against any mock implementing the same shape.
 *
...
/**
 * The capability surface the LangGraph nodes depend on.
 *
 * Keeping this as an interface (rather than depending on the concrete class) is what makes
 * the example graph testable: tests inject a deterministic mock implementing `OabpClient`,
 * production injects {@link OabpSdk}.
 */
export interface OabpClient {
  ...
  a2aSend(message: string, opts?: { taskId?: string }): Promise<A2AResponse>;
 ...
 *
 * Every method maps 1:1 to a documented endpoint and performs a genuine request.
 */
export class OabpSdk implements OabpClient {
   ...
   * Sends a `message/send` with a single text part. Pass `taskId` to continue a task.
   */
  a2aSend(message: string, opts: { taskId?: string } = {}): Promise<A2AResponse> {
