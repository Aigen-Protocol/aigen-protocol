 ...
 *
 * It is the deterministic substrate the tests and the offline example run against:
 ...
 *
 * Submissions are recorded and `/api/stats` reflects them, so a graph run is observable end-to-end.
 */
...
export class MockOabpClient implements OabpClient {
  ...
  constructor(seed: MockSeed = {}) {
    for (const m of seed.missions ?? defaultMissions()) {
      this.missions.set(m.id, structuredCloneSafe(m));
    }
