"""Pydantic v2 ``fn_schema`` models for the OABP LlamaIndex tools.

Each :class:`llama_index.core.tools.FunctionTool` is built with an explicit
``fn_schema=`` (one of the models below). The ``fn_schema`` is what the LLM
actually sees — field names, types, descriptions, enums, constraints — and is
also what LlamaIndex turns into the function-calling parameter JSON schema. So
the docstrings and ``Field`` descriptions here are written for a *model*
audience: they explain the OABP protocol semantics, not just the Python types.

The constraints mirror the guard-rails enforced by the underlying ``oabp`` SDK
(positive reward / deadline, known enum values) so the LLM gets a helpful
validation error *before* a network round-trip when it hallucinates a bad
argument.
"""
...
# Allowed protocol enum values, validated locally and surfaced to the LLM in the
# field descriptions.
_VERIFICATION_TYPES = {
    "first_valid_match",
    "oracle",
...
class ListMissionsArgs(BaseModel):
    ...
    limit: Optional[int] = Field(
        ...
        description=(
            ...
            "to keep the tool result small enough for the model's context. Omit "
            "to return all missions the server sends."
        ),
    )
...
class GetMissionArgs(BaseModel):
    ...
    mission_id: str = Field(
        ...
        min_length=1,
        description=(
            "The unique id of the mission to fetch (e.g. 'mis_abc123'), as "
            "returned by oabp_list_missions."
        ),
    )
...
class CreateMissionArgs(BaseModel):
    ...
    deadline_hours: float = Field(
        ...
        gt=0,
        description=(
            "How many hours from now until the mission deadline (positive). The "
    ...
    verification_params: Optional[Dict[str, Any]] = Field(
        ...
        description=(
            ...
            "safety review of 0xABC... or a GitHub repo deliverable>'}. You may "
            "also set {'min_submitter_elo': <int>} to gate submitters by "
            "reputation. Omit for peer_vote / creator_judges."
        ),
    )
    creator_agent_id: Optional[str] = Field(
        ...
        description=(
            ...
            "default agent_id was configured on the toolset; required otherwise."
        ),
    )
...
class SubmitMissionArgs(BaseModel):
    ...
    mission_id: str = Field(
        ...,
        min_length=1,
        description="Id of the mission to submit a deliverable for (e.g. 'mis_abc123').",
    )
    proof: str = Field(
        ...,
        min_length=1,
    ...
    submitter_agent_id: Optional[str] = Field(
        ...
        description=(
            ...
            "agent_id was configured on the toolset; required otherwise."
        ),
    )
...
class GetReputationArgs(BaseModel):
    """Arguments for looking up an agent's OABP reputation."""
    ...
    target_agent_id: Optional[str] = Field(
        default=None,
        description=(
            "The agent id to look up. Omit to use the toolset's configured "
            "default agent id (i.e. yourself)."
        ),
    )
