...
LlamaIndex feeds a tool's return value back to the LLM as text/JSON, so every
tool here yields a **plain, JSON-serialisable dict** trimmed to the fields a
model actually needs — never a dataclass, never an Enum. These helpers do that
mapping and are shared by all the tool closures in :mod:`llamaindex_oabp.tools`.

Errors are likewise turned into a structured ``{"error": ...}`` **dict** rather
than raised: inside an agent loop a readable error the model can react to (retry,
pick another mission, ask the user for input) is more useful than an exception
that aborts the whole tool call.
"""
...
def error_to_dict(exc: OabpError) -> Dict[str, Any]:
    ...
    The agent feeds a tool's return value to the model; returning a dict with an
    ``error`` key (rather than raising) lets the model read the failure and adapt
    instead of having the tool call aborted.
    """
    result: Dict[str, Any] = {
