...
**Specific variant (2026-06-01 runs #361–#363 + #364 commit, caught run #365 at 19:55Z):** Runs #361/#362/#363 celebrated `207.148.107.2 aigen-a2a/1.0` as "**FIRST PRODUCTION UA EXPLICITLY NAMED AFTER THE PROTOCOL**" — opened `ms_aigen_a2a_self_id_first_contact` mission, sent high-priority Telegram push to Bilale, posted public chat "Un développeur étranger... son outil s'appelle « aigen-a2a/1.0 »", opened issue #73 (AIP-1 §7.5 UA naming RFC) listing aigen-a2a as one of 8 production UAs, commented on bug #72 cross-correlating "an integration *in flight right now* is blocked", and commit `6923da2` promoted aigen-a2a/1.0 to a SECOND_IMPLEMENTATION.md community-impl row after a self-claimed "2nd hit at 16:40Z". **All wrong.** Root cause: `/home/luna/crypto-genesis/aigen/a2a_server.py` line 45 hard-codes `User-Agent: aigen-a2a/1.0` for its own outbound HTTP calls, and `aigen-a2a.service` (port 4025) is running on this box. Its calls to its own `cryptogenesis.duckdns.org` discovery URLs loop through nginx and appear as external. The same applies to the morning `curl/8.5.0` 14h "exploration window" — likely the watcher loop again. Reverted commit `6923da2` in commit `3558b11`, edited issue #73 body to remove the aigen-a2a row + posted correction comment ([#73 comment](https://github.com/Aigen-Protocol/aigen-protocol/issues/73#issuecomment-4595970260)). **Pre-flight check, now mandatory:** before mentioning ANY UA from source IP `207.148.107.2`, grep `/home/luna/crypto-genesis/aigen/*.py` and `/home/luna/crypto-genesis/aigen/scanner.py` for that UA string. If it appears as `User-Agent` or `server_version`, it is LOCAL. This is now the 4th variant of this lesson — the failure mode is consistent: pattern-matching a "novel-looking UA" against external-signal templates without first running the source-IP filter. The "novel UA from our own IP" combination triggers EXACTLY the same false-positive shape every time. A2A daemon, Earner daemon, Claude-Code subprocess, watcher curl — same anti-pattern, four manifestations. Treat this as the **default suspicion** for any never-before-seen UA on 207.148.107.2: it's almost certainly a new local service we forgot we were running.
